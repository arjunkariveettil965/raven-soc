"""Shared synthetic scenario orchestration for Streamlit and FastAPI."""
...
from core.scenario_evaluation import evaluate_scenario_result
...
def _normalize_environment_profile(profile: dict[str, Any], requested_name: str) -> dict[str, Any]:
    ...
    critical_devices = normalized.get("critical_devices") or normalized.get("CriticalDevices") or []
    normalized["EnvironmentName"] = str(environment_name)
    normalized["critical_devices"] = [critical_devices] if isinstance(critical_devices, str) else list(critical_devices)
    return normalized


def _normalize_incident(correlated_incident: pd.Series | dict[str, Any], classified_incident: pd.Series | dict[str, Any]) -> pd.Series:
...
def run_synthetic_incident_pipeline(
    ...
    if scenario_mode == "Random Scenario":
        scenario = generate_random_scenario(seed=seed, difficulty=difficulty, noise_level=noise_level, environment=environment_name)
    elif scenario_name == "multi_stage_intrusion" and seed is None and difficulty == "Medium" and noise_level == "Low":
        events = _as_dataframe(attack_simulator.generate_multistage_attack_scenario(), "synthetic events")
        scenario = {
            ...
            "ExpectedMITRETechniques": ["T1110 - Brute Force", "T1078 - Valid Accounts", "T1059.001 - PowerShell", "T1071 - Application Layer Protocol"],
            "ExpectedTarget": "CFO-PC",
            "ExpectedAlertTypes": ["Failed Login Burst", "Successful Login After Failures", "Suspicious PowerShell", "Suspicious Outbound Connection"],
        ...
        scenario = generate_scenario(scenario_name, seed=seed, difficulty=difficulty, noise_level=noise_level, environment=environment_name)

    events = _as_dataframe(scenario["GeneratedEvents"], "synthetic events")
    alerts = _as_dataframe(alert_engine.analyze_security_events(events=events), "alerts")
    if alerts.empty:
        raise RuntimeError("The synthetic scenario produced no security alerts.")
    correlated_incidents = _as_dataframe(correlation_engine.correlate_alerts(alerts=alerts), "correlated incidents")
    if correlated_incidents.empty:
        raise RuntimeError("The generated alerts produced no correlated incident.")
    classifications = _as_dataframe(incident_classifier.classify_incidents(incidents=correlated_incidents), "classified incidents")
    if classifications.empty:
        raise RuntimeError("Incident classification returned no incident.")

    selected_incident = _normalize_incident(correlated_incident=correlated_incidents.iloc[0], classified_incident=classifications.iloc[0])
    incidents = pd.DataFrame([selected_incident.to_dict()])
    timeline = _as_dataframe(timeline_builder.build_incident_timeline(incident=selected_incident, alerts=alerts), "incident timeline")
    formatted_timeline = list(timeline_builder.format_incident_timeline(timeline=timeline))
    profile = environment_adapter.get_environment_profile(environment_name=environment_name)
    ...
    if analyst_mode in {"ollama", "hybrid"}:
        ...
        ollama_health = {"available": False, "base_url": ollama_base_url, "models": [], "error": "Health check not required in deterministic mode."}
    evaluation = evaluate_scenario_result(scenario=scenario, alerts=alerts, incidents=incidents)
    return {
        "scenario": scenario,
...
def run_synthetic_defender_response(analysis: dict[str, object], environment_profile: dict[str, object], human_approved: bool) -> dict[str, object]:
    result = defender_agent.run_defender_agent(analysis=analysis, environment_profile=environment_profile, human_approved=human_approved)
